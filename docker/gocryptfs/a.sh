...
docker run --rm -it -v /dev/fuse:/dev/fuse --privileged -v $PWD/c:/c:shared -v $PWD/p:/p:ro -v $PWD/passfile:/passfile:ro $i -fg -allow_other -nosyslog -reverse -passfile /passfile /p /c
